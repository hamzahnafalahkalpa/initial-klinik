...
use Projects\Klinik\Concerns\HasApproval;
...
class ProcurementController extends ApiController{
    ...
    protected function procurementSetup(? array $procurement = null){
        $this->userAttempt();
        $warehouse_type = request()->warehouse_type ?? 'Room';
        ...
        $procurement = $this->mergeArray($procurement,[
                        ...
                        ],call_user_func(function() use ($warehouse_type){
                            if (isset(request()->room_id)){
                                return [
                                    'warehouse_type' => $warehouse_type,
                                    'warehouse_id'   => request()->room_id
                                ];
                            }
                            return [];
                        }));
        if (isset($procurement['card_stocks']) && count($procurement['card_stocks']) > 0) {
            ...
            foreach ($card_stocks as &$card_stock) {
                $card_stock['stock_movement']['qty']            = floatval($card_stock['qty'] ?? 0);
                $card_stock['stock_movement']['cogs']           = floatval($card_stock['cogs'] ?? 0);
                $card_stock['stock_movement']['reference_type'] = $procurement['warehouse_type'] ?? $warehouse_type;
                $card_stock['stock_movement']['reference_id']   = $procurement['warehouse_id'] ?? request()->room_id;
            }
        }
        return $procurement;
    }
    ...
    protected function purchasingProcurementSetup(array $purchase_orders){
        ...
        foreach ($purchase_orders as &$purchase_order) {
            ...
            if (isset(request()->room_id)){
                $procurement['warehouse_type'] = request()->warehouse_type ?? 'Room';
